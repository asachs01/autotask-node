...
export class AutotaskErrorHandler extends EventEmitter {
  ...
  // Recovery tracking
  private activeRecoveries: Map<
    string,
    { strategy: ErrorRecoveryStrategy; startTime: Date; step: number }
  > = new Map();
  ...
  async handleError(
    ...
    if (error instanceof AutotaskError) {
      ...
      autotaskError = this.createAutotaskErrorFromAxios(
        error as AxiosError,
        context
      );
    } else {
  ...
  suggestRecoveryActions(
    ...
    if (context.entityType) {
      suggestions.push(
        `Review ${context.entityType} configuration and permissions`
      );
    }
    ...
    if (error instanceof RateLimitError) {
      ...
      suggestions.push(
        'Review API usage patterns and optimize batch operations'
      );
    }
  ...
  getErrorStatistics(timeRange: number = 3600000): Record<string, any> {
    ...
    recentErrors.forEach(entry => {
      ...
      stats.errorsByEndpoint.set(
        endpoint,
        (stats.errorsByEndpoint.get(endpoint) || 0) + 1
      );
  ...
  private extractFromErrorMessage(
    ...
    if (message.includes('zone') || message.includes('webservice')) {
      return {
        ...
        isRetryable: true,
        recommendedAction:
          'Retry zone detection or use manual zone configuration',
        autotaskSpecific: true,
      };
    }
  ...
  private classifyByStatusCode(
    ...
    if (statusCode >= 500) {
      ...
      return {
        ...
        isRetryable: true,
        recommendedAction:
          'Review error details and implement appropriate handling',
        autotaskSpecific: false,
      };
    }
  }
  ...
  private updateErrorPatterns(): void {
    ...
    const errorGroups = new Map<
      string,
      Array<{ error: AutotaskError; context: ErrorContext }>
    >();
    ...
    for (const [errorType, entries] of errorGroups.entries()) {
      ...
      pattern.escalationRequired =
        ...
        entries.some(
          entry =>
            this.classifyError(entry.error, entry.context).severity ===
            'CRITICAL'
        );
  ...
  private initializeErrorMappings(): void {
    ...
    this.errorMap.set('ServerError:503', {
      ...
      isRetryable: true,
      recommendedAction:
        'Retry with exponential backoff, consider failover to backup zone',
      autotaskSpecific: true,
    });
  }
