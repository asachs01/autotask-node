...
export interface LoadTestResult {
  ...
  resourceUtilization: {
    ...
    connections: Array<{ timestamp: number; active: number; idle: number }>;
  };
  /** Errors encountered */
  errors: Array<{
    timestamp: number;
...
export interface BenchmarkComparison {
  ...
  changes: {
    ...
    latency: {
      average: { absolute: number; percentage: number };
      p95: { absolute: number; percentage: number };
      p99: { absolute: number; percentage: number };
      significance: 'improved' | 'degraded' | 'unchanged';
    };
    memory: {
      peak: { absolute: number; percentage: number };
      average: { absolute: number; percentage: number };
      growthRate: { absolute: number; percentage: number };
      significance: 'improved' | 'degraded' | 'unchanged';
    };
    errorRate: {
      absolute: number;
...
export interface BenchmarkProfile {
  ...
  entityTypes: string[];
  /** Operations mix */
  operationsMix: Record<string, number>; // operation -> percentage
  ...
  dataCharacteristics: {
    ...
    dataVariation: number; // 0-1 (0 = identical, 1 = completely random)
  };
