...
export class MemoryBenchmark extends EventEmitter {
   ...
   */
  async run(config: BenchmarkConfig): Promise<MemoryResult> {
    if (this.isRunning) {
  ...
  private async runWarmup(config: BenchmarkConfig): Promise<void> {
    const warmupIterations =
      config.warmupIterations || Math.floor((config.iterations || 100) * 0.1);
   ...
   */
  private async runBenchmarkPhase(config: BenchmarkConfig): Promise<void> {
  ...
  private calculateResults(
    initialMemory: { heapUsed: number; heapTotal: number; rss: number },
    finalMemory: { heapUsed: number; heapTotal: number; rss: number },
    durationMs: number
  ): MemoryResult {
   ...
   * Detect memory leak indicators
   */
  private detectMemoryLeak(growthRate: number): {
    suspected: boolean;
