 ...
 */
export class TicketBusinessLogic {
   ...
   */
  async createTicket(ticketData: any, context?: {
    user?: any;
  ...
  calculateSLADueDate(
    ...
    const defaultSlaForPriority = defaultSLA[priority as keyof typeof defaultSLA] || defaultSLA[this.TICKET_PRIORITIES.MEDIUM];
  ...
  checkEscalationRules(ticket: any): {
    ...
    if (ticket.dueDateTime && new Date(ticket.dueDateTime) < now) {
      const hoursOverdue = Math.floor(
        (now.getTime() - new Date(ticket.dueDateTime).getTime()) / (1000 * 60 * 60)
      );
    ...
    if (ticket.status === this.TICKET_STATUSES.WAITING_CUSTOMER && ticket.lastActivityDate) {
      const daysSinceActivity = Math.floor(
        (now.getTime() - new Date(ticket.lastActivityDate).getTime()) / (1000 * 60 * 60 * 24)
      );
  ...
  calculateTicketMetrics(tickets: any[]): {
    ...
    tickets.forEach(ticket => {
      const statusKey = Object.keys(this.TICKET_STATUSES).find(
        key => this.TICKET_STATUSES[key as keyof typeof this.TICKET_STATUSES] === ticket.status
      ) || 'UNKNOWN';
    ...
    tickets.forEach(ticket => {
      const priorityKey = Object.keys(this.TICKET_PRIORITIES).find(
        key => this.TICKET_PRIORITIES[key as keyof typeof this.TICKET_PRIORITIES] === ticket.priority
      ) || 'UNKNOWN';
   ...
   */
  private applyTicketCreationLogic(ticketData: any, context?: any): any {
   ...
   */
  private getStatusRequirements(newStatus: number, currentTicket: any): {
