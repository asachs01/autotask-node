...
export class ExternalLogHandler implements LogDestinationHandler {
  constructor(
    private readonly config: NonNullable<ErrorLoggerConfig['external']>
  ) {}
  ...
  async write(entry: LogEntry): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `Failed to send log to external service: ${response.status}`
        );
      }
    } catch (error) {
...
export class ErrorLogger {
  ...
  private readonly handlers: Map<LogDestination, LogDestinationHandler> =
    new Map();
  ...
  logRetry(
    ...
    this.warn(
      `Retry attempt ${retryInfo.attempt}/${retryInfo.maxAttempts}: ${message}`,
      error,
      {
        ...context,
        retry: {
    ...
    for (const destination of this.config.destinations) {
      switch (destination) {
        case LogDestination.CONSOLE:
          this.handlers.set(
            destination,
            new ConsoleLogHandler(this.config.jsonFormat)
          );
          break;
        ...
        case LogDestination.FILE:
          this.handlers.set(
            destination,
            new FileLogHandler(this.config.filePath, true)
          );
          break;
        ...
        case LogDestination.EXTERNAL:
          if (this.config.external.endpoint) {
            this.handlers.set(
              destination,
              new ExternalLogHandler(this.config.external)
            );
          }
          break;
      }
...
export function LogErrors(
  ...
  return function (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        defaultErrorLogger.debug(message || `Executing ${methodName}`, {
          ...context,
          correlationId,
        ...
        defaultErrorLogger.debug(`Successfully completed ${methodName}`, {
          ...context,
          correlationId,
          operation: methodName,
          performance: {
