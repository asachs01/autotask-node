...
export class BatchManager extends EventEmitter {
  ...
  private isDuplicate(batch: QueueBatch, newRequest: QueueRequest): boolean {
    ...
    return batch.requests.some(
      existingRequest => existingRequest.fingerprint === newRequest.fingerprint
    );
  }
  ...
  private getDynamicBatchSize(batchKey: string): number {
    ...
    let dynamicSize =
      this.dynamicBatchSizes.get(batchKey) || this.options.maxBatchSize;
  ...
  private adjustBatchSizes(): void {
    ...
    for (const [key, performances] of performanceByKey) {
      const avgProcessingTime =
        performances.reduce((sum, p) => sum + p.processingTime, 0) /
        performances.length;
      const successRate =
        performances.filter(p => p.success).length / performances.length;

      let currentSize =
        this.dynamicBatchSizes.get(key) || this.options.maxBatchSize;
  ...
  private updateStatistics(): void {
    ...
    this.stats.averageBatchSize =
      recentBatches.reduce(
        (sum, entry) => sum + entry.batch.requests.length,
        0
    ...
    this.stats.averageWaitTime =
      recentBatches.reduce(
        (sum, entry) =>
          sum + (entry.endTime.getTime() - entry.batch.createdAt.getTime()),
        0
      ) / recentBatches.length;
    ...
    this.stats.averageProcessingTime =
      recentBatches.reduce(
        (sum, entry) =>
          sum + (entry.endTime.getTime() - entry.startTime.getTime()),
        0
      ) / recentBatches.length;
    ...
    const timeoutBatches = recentBatches.filter(
      entry => entry.batch.requests.length < entry.batch.maxSize
    ).length;
  ...
  private startLoadMonitoring(): void {
    ...
    setInterval(() => {
      ...
      const avgBatchAge =
        Array.from(this.activeBatches.values()).reduce(
  ...
  markBatchCompleted(
    ...
    const historyEntry = this.batchHistory.find(
      entry => entry.batch.id === batchId
    );
    if (historyEntry) {
      historyEntry.success = success;
      historyEntry.endTime = new Date(
        historyEntry.startTime.getTime() + processingTime
      );
    }
  }
}
