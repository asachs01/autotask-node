...
import { BatchResultBuilder, BatchResultAnalyzer } from './BatchResult';
import { BatchMetricsCollector } from './BatchMetrics';
import { BatchStrategyFactory } from './BatchStrategy';
import { CircuitBreaker } from '../errors/CircuitBreaker';
import { RetryStrategy } from '../errors/RetryStrategy';
import { ErrorLogger, LogLevel, LogDestination } from '../errors/ErrorLogger';

/**
 * Active batch tracking information
 */
...
export class BatchManager extends EventEmitter {
  ...
  private readonly metricsCollector: BatchMetricsCollector;
  private readonly resultAnalyzer: BatchResultAnalyzer;
  private readonly retryStrategy: RetryStrategy;
  private readonly circuitBreaker: CircuitBreaker;
  
  // Strategy management
  private readonly strategies = new Map<string, IBatchStrategy>();
  private currentStrategy: IBatchStrategy;
  ...
  private readonly processors = new Map<string, IBatchProcessor>();
  ...
  constructor(
    ...
    this.metricsCollector = new BatchMetricsCollector(300000, 1000, this.logger);
    this.resultAnalyzer = new BatchResultAnalyzer(this.logger);
    ...
    this.circuitBreaker = new CircuitBreaker('batch-processing', {
   ...
   */
  async addRequest(request: BatchRequest): Promise<void> {
    ...
    try {
      ...
      this.errorLogger.error('Failed to add request to batch', error as Error, {
  ...
  async processBatch(batchId: string): Promise<BatchResult> {
    ...
    try {
      ...
      this.metricsCollector.recordEndpointPerformance(
        optimizedBatch.endpoint,
        optimizedBatch.requests.length,
      ...
      this.errorLogger.error('Failed to process batch', error as Error, {
        correlationId: this.errorLogger.generateCorrelationId(),
      ...
      const failureResult = new BatchResultBuilder(batchId, activeBatch.strategy.name)
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      const shutdownPromises = Array.from(this.activeBatches.keys()).map(batchId =>
        this.processBatch(batchId).catch(error =>
          this.logger.error('Error processing batch during shutdown', {
            batchId,
  ...
  private initializeStrategies(): void {
    ...
    this.strategies.set('size-based', BatchStrategyFactory.create('size-based', strategyConfig));
    this.strategies.set('time-based', BatchStrategyFactory.create('time-based', strategyConfig));
    this.strategies.set('hybrid', BatchStrategyFactory.create('hybrid', strategyConfig));
    this.strategies.set('priority-aware', BatchStrategyFactory.create('priority-aware', strategyConfig));
    this.strategies.set('adaptive', BatchStrategyFactory.create('adaptive', strategyConfig));
  }
  ...
  private findCompatibleBatch(batchKey: string, request: BatchRequest): ActiveBatch | undefined {
  ...
  private async createNewBatch(batchKey: string, initialRequest: BatchRequest): Promise<ActiveBatch> {
    ...
    const timeout = setTimeout(() => {
      this.processBatch(batchId).catch(error =>
        this.logger.error('Error auto-processing batch', {
          batchId,
          error: error instanceof Error ? error.message : error
        })
      );
  ...
  private isRequestCompatible(request: BatchRequest, activeBatch: ActiveBatch): boolean {
    const batch = activeBatch.batch;
    
    // Basic compatibility checks
    if (request.zone !== batch.zone) return false;
    if (request.endpoint !== batch.endpoint) return false;
    if (request.method !== (batch.requests[0]?.method)) return false;
    
    // Priority compatibility (within 2 levels)
    if (Math.abs(request.priority - batch.priority) > 2) return false;
    
    // Check batch hints
    const hints = request.batchHints;
    if (hints?.maxDelay) {
      const batchAge = Date.now() - activeBatch.createdAt.getTime();
      if (batchAge > hints.maxDelay) return false;
    }
    
    return true;
  }
  
  private selectStrategyForRequest(request: BatchRequest): IBatchStrategy {
  ...
  private async processQueuedBatch(batch: QueueBatch): Promise<BatchResult> {
  ...
  private findProcessor(batch: QueueBatch): IBatchProcessor {
  ...
  private async handleFailedRequests(result: BatchResult): Promise<void> {
  ...
  private async processIndividualRequest(request: BatchRequest): Promise<void> {
    ...
    try {
      ...
      this.errorLogger.error('Failed to process individual request', error as Error, {
        correlationId: this.errorLogger.generateCorrelationId(),
