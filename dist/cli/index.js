...
const cwdEnvPath = (0, path_1.resolve)(process.cwd(), '.env');
...
async function main() {
    ...
    if (command === 'delete' && !DELETABLE_ENTITIES.has(entity)) {
        console.error(`Error: Entity '${entity}' does not support DELETE operations.`);
        console.error(`DELETE is only supported for: ${Array.from(DELETABLE_ENTITIES).join(', ')}`);
        return printUsage();
    }
    const auth = {
    ...
    let client;
    try {
        client = await AutotaskClient_1.AutotaskClient.create(auth);
    }
    catch (err) {
        ...
        if (entity === 'tickets') {
            ...
            switch (command) {
                case 'create': {
                    ...
                    const res = await client.contractServiceAdjustments.create(contractServiceAdjustment);
                    return console.log(JSON.stringify(res, null, 2));
                }
                case 'get': {
                    const id = Number(args[0]);
                ...
                case 'update': {
                    ...
                    const res = await client.contractServiceAdjustments.update(id, contractServiceAdjustment);
                    return console.log(JSON.stringify(res, null, 2));
                }
                case 'delete': {
                    const id = Number(args[0]);
            ...
            switch (command) {
                case 'create': {
                    ...
                    const res = await client.contractExclusionSets.create(contractExclusionSet);
                    return console.log(JSON.stringify(res, null, 2));
                }
                case 'get': {
                    const id = Number(args[0]);
                ...
                case 'update': {
                    ...
                    const res = await client.contractExclusionSets.update(id, contractExclusionSet);
                    return console.log(JSON.stringify(res, null, 2));
                }
                case 'delete': {
                    const id = Number(args[0]);
