 ...
 */
class PriceListRoles extends base_1.BaseEntity {
    constructor(axios, logger, requestHandler) {
        super(axios, logger, requestHandler);
        this.endpoint = '/PriceListRoles';
    }
    static getMetadata() {
        return [
            {
     ...
     */
    async create(priceListRoles) {
        this.logger.info('Creating pricelistroles', { priceListRoles });
        return this.executeRequest(async () => this.axios.post(this.endpoint, priceListRoles), this.endpoint, 'POST');
    }
    /**
     ...
     */
    async update(id, priceListRoles) {
        this.logger.info('Updating pricelistroles', { id, priceListRoles });
        return this.executeRequest(async () => this.axios.put(`${this.endpoint}/${id}`, priceListRoles), `${this.endpoint}/${id}`, 'PUT');
    }
    /**
     ...
     * @returns Promise with the updated pricelistroles
     */
    async patch(id, priceListRoles) {
        this.logger.info('Patching pricelistroles', { id, priceListRoles });
        return this.executeRequest(async () => this.axios.patch(`${this.endpoint}/${id}`, priceListRoles), `${this.endpoint}/${id}`, 'PATCH');
    }
    /**
...
exports.PriceListRoles = PriceListRoles;
//# sourceMappingURL=pricelistroles.js.map