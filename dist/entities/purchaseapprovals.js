...
exports.PurchaseApprovals = void 0;
 ...
 */
class PurchaseApprovals extends base_1.BaseEntity {
    constructor(axios, logger, requestHandler) {
        super(axios, logger, requestHandler);
        this.endpoint = '/PurchaseApprovals';
    }
    static getMetadata() {
        return [
            {
     ...
     */
    async create(purchaseApprovals) {
        this.logger.info('Creating purchaseapprovals', { purchaseApprovals });
        return this.executeRequest(async () => this.axios.post(this.endpoint, purchaseApprovals), this.endpoint, 'POST');
    }
    /**
     ...
     */
    async update(id, purchaseApprovals) {
        this.logger.info('Updating purchaseapprovals', { id, purchaseApprovals });
        return this.executeRequest(async () => this.axios.put(`${this.endpoint}/${id}`, purchaseApprovals), `${this.endpoint}/${id}`, 'PUT');
    }
    /**
     ...
     * @returns Promise with the updated purchaseapprovals
     */
    async patch(id, purchaseApprovals) {
        this.logger.info('Patching purchaseapprovals', { id, purchaseApprovals });
        return this.executeRequest(async () => this.axios.patch(`${this.endpoint}/${id}`, purchaseApprovals), `${this.endpoint}/${id}`, 'PATCH');
    }
    /**
...
exports.PurchaseApprovals = PurchaseApprovals;
//# sourceMappingURL=purchaseapprovals.js.map