 ...
 */
class ContactGroups extends base_1.BaseEntity {
    constructor(axios, logger, requestHandler) {
        super(axios, logger, requestHandler);
        this.endpoint = '/ContactGroups';
    }
    static getMetadata() {
        return [
            {
     ...
     */
    async create(contactGroups) {
        this.logger.info('Creating contactgroups', { contactGroups });
        return this.executeRequest(async () => this.axios.post(this.endpoint, contactGroups), this.endpoint, 'POST');
    }
    /**
     ...
     */
    async update(id, contactGroups) {
        this.logger.info('Updating contactgroups', { id, contactGroups });
        return this.executeRequest(async () => this.axios.put(`${this.endpoint}/${id}`, contactGroups), `${this.endpoint}/${id}`, 'PUT');
    }
    /**
     ...
     * @returns Promise with the updated contactgroups
     */
    async patch(id, contactGroups) {
        this.logger.info('Patching contactgroups', { id, contactGroups });
        return this.executeRequest(async () => this.axios.patch(`${this.endpoint}/${id}`, contactGroups), `${this.endpoint}/${id}`, 'PATCH');
    }
    /**
