 ...
 */
class QuoteLocations extends base_1.BaseEntity {
    constructor(axios, logger, requestHandler) {
        super(axios, logger, requestHandler);
        this.endpoint = '/QuoteLocations';
    }
    static getMetadata() {
        return [
            {
     ...
     */
    async create(quoteLocations) {
        this.logger.info('Creating quotelocations', { quoteLocations });
        return this.executeRequest(async () => this.axios.post(this.endpoint, quoteLocations), this.endpoint, 'POST');
    }
    /**
     ...
     */
    async update(id, quoteLocations) {
        this.logger.info('Updating quotelocations', { id, quoteLocations });
        return this.executeRequest(async () => this.axios.put(`${this.endpoint}/${id}`, quoteLocations), `${this.endpoint}/${id}`, 'PUT');
    }
    /**
     ...
     * @returns Promise with the updated quotelocations
     */
    async patch(id, quoteLocations) {
        this.logger.info('Patching quotelocations', { id, quoteLocations });
        return this.executeRequest(async () => this.axios.patch(`${this.endpoint}/${id}`, quoteLocations), `${this.endpoint}/${id}`, 'PATCH');
    }
    /**
...
exports.QuoteLocations = QuoteLocations;
//# sourceMappingURL=quotelocations.js.map