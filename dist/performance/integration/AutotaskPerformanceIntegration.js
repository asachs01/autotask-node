...
exports.AutotaskPerformanceIntegration = void 0;
const PerformanceMonitor_1 = require("../monitoring/PerformanceMonitor");
const IntelligentCache_1 = require("../caching/IntelligentCache");
const RequestOptimizer_1 = require("../optimization/RequestOptimizer");
const BenchmarkSuite_1 = require("../benchmarks/BenchmarkSuite");
/**
 ...
 */
class AutotaskPerformanceIntegration {
    constructor(axios, logger, config = {}) {
        ...
        this.performanceMonitor = new PerformanceMonitor_1.PerformanceMonitor(logger, config.performance);
        this.cache = new IntelligentCache_1.IntelligentCache(logger, config.cache);
        this.requestOptimizer = new RequestOptimizer_1.RequestOptimizer(logger, config.optimization);
        this.benchmarkSuite = new BenchmarkSuite_1.BenchmarkSuite(logger);
    ...
    getDashboard() {
        ...
        const recommendations = this.generateComprehensiveRecommendations(performance, cache, optimization);
        return {
            performance,
            cache,
            optimization,
     ...
     */
    async benchmarkEntities(entityTypes, config) {
    ...
    calculateResponseTTL(url, data) {
        ...
        if (url.match(/\/(tickets|tasks|opportunities)/) && data?.items?.some((item) => ['New', 'In Progress', 'Waiting'].includes(item.status))) {
            return 60000; // 1 minute
        }
        return 300000; // 5 minutes default
    }
    convertToBatchRequest(config) {
        return {
            id: config.metadata?.requestId || 'unknown',
    ...
    convertToAxiosResponse(batchResponse, config) {
        return {
    ...
    recordCacheHit(config, response) {
        const requestMetrics = {
            requestId: config.metadata?.requestId || 'cached',
            method: config.method?.toUpperCase() || 'GET',
            endpoint: this.extractEndpoint(config.url || ''),
    ...
    createEntityBenchmarkScenarios(entityTypes) {
    ...
    getOptimalQueryParams(entityType, operation) {
        const baseParams = {
            pageSize: 100,
    ...
    calculateOptimalTTL(frequency) {
        ...
        if (frequency > 100)
            return 30000; // 30 seconds for very frequent
        if (frequency > 50)
            return 120000; // 2 minutes for frequent
        if (frequency > 10)
            return 300000; // 5 minutes for moderate
        return 600000; // 10 minutes for infrequent
    }
    generateComprehensiveRecommendations(performance, cache, optimization) {
...
exports.AutotaskPerformanceIntegration = AutotaskPerformanceIntegration;
//# sourceMappingURL=AutotaskPerformanceIntegration.js.map