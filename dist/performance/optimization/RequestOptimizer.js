...
exports.RequestOptimizer = void 0;
 ...
 */
class RequestOptimizer extends events_1.EventEmitter {
    constructor(logger, config = {}) {
        ...
        this.batchProcessor = new BatchProcessor_1.BatchProcessor(logger, this.config);
        this.deduplicator = new RequestDeduplicator_1.RequestDeduplicator(logger, this.config);
        this.compressionManager = new CompressionManager_1.CompressionManager(logger, this.config);
        ...
        try {
            ...
            const processingPromises = optimizedGroups.map(async (group) => {
                if (this.config.enableBatching && group.length > 1) {
                    ...
                    return await Promise.all(group.map(req => this.optimizeRequest(req)));
                }
            });
    ...
    setupEventHandlers() {
        this.batchProcessor.on('batch_processed', (batchInfo) => {
    ...
    initializeDefaultRules() {
        ...
        this.addOptimizationRule({
            ...
            condition: (request) => request.method === 'GET' && request.endpoint.includes('list'),
            action: async (request) => {
                request.metadata = {
                    ...request.metadata,
    ...
    async groupRequestsForOptimization(requests) {
        const groups = [];
...
//# sourceMappingURL=RequestOptimizer.js.map