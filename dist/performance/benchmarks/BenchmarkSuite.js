...
const ThroughputBenchmark_1 = require("./ThroughputBenchmark");
const LatencyBenchmark_1 = require("./LatencyBenchmark");
const MemoryBenchmark_1 = require("./MemoryBenchmark");
const LoadTester_1 = require("./LoadTester");
/**
 ...
 */
class BenchmarkSuite extends events_1.EventEmitter {
    constructor(logger) {
        super();
        this.logger = logger;
        this.benchmarkHistory = [];
        this.isRunning = false;
        this.throughputBenchmark = new ThroughputBenchmark_1.ThroughputBenchmark(logger);
        this.latencyBenchmark = new LatencyBenchmark_1.LatencyBenchmark(logger);
        this.memoryBenchmark = new MemoryBenchmark_1.MemoryBenchmark(logger);
        this.loadTester = new LoadTester_1.LoadTester(logger);
    ...
    async runBenchmark(config) {
        ...
        try {
            ...
            if ((config.concurrency || 1) > 1) {
                ...
                loadTestResult = this.createSyntheticLoadTestResult(config, throughputResult, latencyResult, memoryResult);
            }
     ...
     */
    compareBenchmarks(baselineId, currentId) {
    ...
    getPerformanceRecommendations() {
        ...
        recommendations.optimizations.push('Consider implementing request batching for improved throughput', 'Enable response compression to reduce bandwidth usage', 'Implement intelligent caching for frequently accessed data', 'Use connection pooling for better resource utilization');
        return recommendations;
    }
    /**
     * Detect performance regressions
     */
    detectRegressions(currentResult) {
        const regressions = [];
        ...
        const throughputDrop = ((baseline.performance.throughput.requestsPerSecond - current.performance.throughput.requestsPerSecond)
        ...
        // Latency regression
        const latencyIncrease = ((current.performance.latency.average - baseline.performance.latency.average)
    ...
    setupEventHandlers() {
        this.throughputBenchmark.on('progress', (progress) => {
            this.emit('throughput_progress', progress);
        });
        this.latencyBenchmark.on('progress', (progress) => {
            this.emit('latency_progress', progress);
        });
        this.memoryBenchmark.on('progress', (progress) => {
            this.emit('memory_progress', progress);
        });
        this.loadTester.on('progress', (progress) => {
    ...
    createScenariosFromProfile(profile) {
        const scenarios = [];
    ...
    createSyntheticLoadTestResult(config, throughputResult, latencyResult, memoryResult) {
        const now = Date.now();
    ...
    createBenchmarkComparison(baseline, current) {
        const throughputChange = this.calculatePercentageChange(baseline.performance.throughput.requestsPerSecond, current.performance.throughput.requestsPerSecond);
        const latencyChangeAvg = this.calculatePercentageChange(baseline.performance.latency.average, current.performance.latency.average, true // Inverse for latency (lower is better)
        );
        const memoryChangePeak = this.calculatePercentageChange(baseline.performance.memory.peakUsage, current.performance.memory.peakUsage, true);
        const errorRateChange = this.calculatePercentageChange(baseline.summary.errorRate, current.summary.errorRate, true);
        ...
        return {
            ...
            changes: {
                ...
                latency: {
                    ...
                    p95: this.calculatePercentageChange(baseline.performance.latency.percentiles.p95, current.performance.latency.percentiles.p95, true),
                    p99: this.calculatePercentageChange(baseline.performance.latency.percentiles.p99, current.performance.latency.percentiles.p99, true),
                    significance: this.getSignificance(latencyChangeAvg.percentage, true)
                },
                memory: {
                    peak: memoryChangePeak,
                    average: this.calculatePercentageChange(baseline.performance.memory.averageUsage, current.performance.memory.averageUsage, true),
                    growthRate: this.calculatePercentageChange(baseline.performance.memory.growthRate, current.performance.memory.growthRate, true),
    ...
    calculatePercentageChange(baseline, current, inverse = false) {
    ...
    getSignificance(percentage, inverse) {
