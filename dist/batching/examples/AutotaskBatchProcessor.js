...
exports.AutotaskBatchProcessor = void 0;
const BatchResult_1 = require("../BatchResult");
/**
 * Batch processor implementation for Autotask API
 */
class AutotaskBatchProcessor {
    constructor(httpClient, baseUrl, logger) {
        this.isHealthy = true;
    ...
    async processBatch(batch) {
        ...
        const resultBuilder = new BatchResult_1.BatchResultBuilder(batch.id, 'autotask-api');
        ...
        try {
            ...
            for (const request of batch.requests) {
                resultBuilder.addFailure(request.id, error, 500, (Date.now() - startTime) / batch.requests.length);
            }
    ...
    canProcess(batch) {
        ...
        const hasValidMethods = batch.requests.every(req => supportedMethods.includes(req.method));
     ...
     */
    async processMethodGroup(method, requests, resultBuilder) {
        switch (method.toLowerCase()) {
            ...
            default:
                ...
                for (const request of requests) {
                    resultBuilder.addFailure(request.id, new Error(`Unsupported HTTP method: ${method}`), 400, 0);
                }
        }
    }
    /**
     ...
     */
    async processCreateRequests(requests, resultBuilder) {
        ...
        try {
            ...
            this.handleBatchResponse(response.data, requests, resultBuilder, processingTime / requests.length);
        }
        catch (error) {
            ...
            for (const request of requests) {
                resultBuilder.addFailure(request.id, error, this.extractStatusCode(error), avgProcessingTime);
            }
        }
    }
    /**
     ...
     */
    async processUpdateRequests(requests, resultBuilder) {
        ...
        for (const chunk of chunks) {
            const updatePromises = chunk.map(async (request) => {
                const startTime = Date.now();
                try {
                    const response = await this.httpClient.put(`${request.endpoint}/${request.data.id}`, request.data);
                    const processingTime = Date.now() - startTime;
                    const responseSize = this.estimateResponseSize(response.data);
                    resultBuilder.addSuccess(request.id, response.data, response.status, processingTime, responseSize);
                }
                catch (error) {
                    const processingTime = Date.now() - startTime;
                    resultBuilder.addFailure(request.id, error, this.extractStatusCode(error), processingTime);
                }
            });
     ...
     */
    async processQueryRequests(requests, resultBuilder) {
     ...
     */
    async processQueryGroup(endpoint, requests, resultBuilder) {
    ...
    canCombineQueries(requests) {
        ...
        const firstRequest = requests[0];
        return requests.every(req => req.endpoint === firstRequest.endpoint &&
     ...
     */
    async processCombinedQuery(endpoint, requests, resultBuilder) {
        try {
            const startTime = Date.now();
            ...
            this.distributeQueryResults(response.data, requests, resultBuilder, processingTime / requests.length);
        }
        catch (error) {
     ...
     */
    async processIndividualQueries(requests, resultBuilder) {
     ...
     */
    async processDeleteRequests(requests, resultBuilder) {
        ...
        for (const chunk of chunks) {
            const deletePromises = chunk.map(async (request) => {
                ...
                try {
                    ...
                    const processingTime = Date.now() - startTime;
                    resultBuilder.addSuccess(request.id, response.data || { deleted: true }, response.status, processingTime, this.estimateResponseSize(response.data));
                }
                catch (error) {
                    const processingTime = Date.now() - startTime;
                    resultBuilder.addFailure(request.id, error, this.extractStatusCode(error), processingTime);
     ...
     */
    handleBatchResponse(response, requests, resultBuilder, avgProcessingTime) {
        ...
        for (let i = 0; i < requests.length; i++) {
            ...
            if (i < items.length && items[i]) {
                ...
                resultBuilder.addSuccess(request.id, items[i], 200, avgProcessingTime, responseSize);
            }
            else {
                ...
                resultBuilder.addFailure(request.id, new Error(error.message), 400, avgProcessingTime);
            }
        }
    }
    /**
     ...
     */
    distributeQueryResults(response, requests, resultBuilder, avgProcessingTime) {
        const items = response.items || [];
        ...
        for (const request of requests) {
            const relevantItems = items.filter(item => this.isItemRelevantToRequest(item, request));
            const responseSize = this.estimateResponseSize(relevantItems);
            resultBuilder.addSuccess(request.id, { items: relevantItems }, 200, avgProcessingTime, responseSize);
        }
    }
    ...
    isItemRelevantToRequest(item, request) {
...
exports.AutotaskBatchProcessor = AutotaskBatchProcessor;
//# sourceMappingURL=AutotaskBatchProcessor.js.map