...
class PriorityScheduler {
     ...
     */
    recordExecution(request, success, responseTime) {
     ...
     */
    selectFIFO(requests) {
        return requests.reduce((oldest, current) => current.createdAt < oldest.createdAt ? current : oldest);
    }
    /**
     * LIFO scheduling - newest requests first
     */
    selectLIFO(requests) {
        return requests.reduce((newest, current) => current.createdAt > newest.createdAt ? current : newest);
    }
    /**
     * Priority scheduling - highest priority first, then FIFO within priority
     */
    selectPriority(requests) {
    ...
    selectWeighted(requests) {
        ...
        for (const [priority, groupRequests] of priorityGroups) {
            ...
            const oldestInGroup = groupRequests.reduce((oldest, current) => current.createdAt < oldest.createdAt ? current : oldest);
        ...
        for (const group of weightedGroups) {
            ...
            if (random <= currentWeight) {
                ...
                return group.requests.reduce((oldest, current) => current.createdAt < oldest.createdAt ? current : oldest);
            }
        }
