...
class SQLiteBackend {
    constructor(dbPath, options, logger) {
        this.db = null;
    ...
    async dequeue(zone) {
      ...
      `;
            ...
            await this.db.run('UPDATE requests SET status = ? WHERE id = ?', ['processing', request.id]);
            this.invalidateMetricsCache();
            return request;
        }
        catch (error) {
    ...
    async getRequests(filter) {
        ...
        try {
            ...
            return rows.map((row) => this.dbRowToRequest(row));
        }
        catch (error) {
