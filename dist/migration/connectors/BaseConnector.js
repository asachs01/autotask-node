...
class BaseConnector extends events_1.EventEmitter {
    constructor(system, config, options = {}) {
        ...
        this.logger = (0, logger_1.createLogger)(`${system}Connector`);
    }
    /**
     * Fetch all records for an entity
     */
    async fetchAll(entityType, options) {
        const allRecords = [];
     ...
     */
    async fetchIncremental(entityType, since, options) {
     ...
     */
    async validateDataQuality(entityType) {
        try {
     ...
     */
    async exportData(entityType, format, outputPath, options) {
    ...
    updateRateLimit(remaining, resetTime) {
        this.rateLimitRemaining = remaining;
        this.rateLimitReset = resetTime;
    }
    async retryWithBackoff(operation, attempts = this.options.retryAttempts || 3) {
    ...
    async analyzeDataQuality(schema, records) {
        const issues = [];
        ...
        for (const field of schema.fields) {
            if (field.format) {
                const invalidRecords = records.filter(r => r[field.name] && !this.validateFieldFormat(r[field.name], field.format));
    ...
    calculateValidity(schema, records) {
    ...
    async exportToXML(records, outputPath) {
        const fs = await Promise.resolve().then(() => __importStar(require('fs/promises')));
