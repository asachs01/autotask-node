...
const DeliveryManager_1 = require("./reliability/DeliveryManager");
const EventStore_1 = require("./reliability/EventStore");
const SynchronizationPatterns_1 = require("./patterns/SynchronizationPatterns");
class WebhookManager extends events_1.EventEmitter {
    constructor(config, logger) {
        ...
        this.parser = new EventParser_1.EventParser({
            validateSchema: true,
            enrichEvents: true,
            generateIds: true,
    ...
    async initialize() {
        ...
        try {
            ...
            if (this.config.server) {
                this.receiver = new WebhookReceiver_1.WebhookReceiver(this.config.server, this.config.security, this.logger);
                // Set up receiver to process webhooks through our pipeline
                this.receiver
                    .getApp()
                    .post(this.config.server.path || '/webhook', async (req, res) => {
                    try {
            ...
            if (this.config.delivery && this.config.eventStore?.type === 'redis') {
                this.deliveryManager = new DeliveryManager_1.DeliveryManager({
            ...
            if (this.config.eventStore) {
                this.eventStore = new EventStore_1.EventStore({
                    ...
                    enableIndexing: true,
                    persistenceMode: this.config.eventStore.type,
                }, this.logger);
            }
            // Initialize synchronization engine
            this.syncEngine = new SynchronizationPatterns_1.SynchronizationEngine(this.logger);
    ...
    async processWebhook(payload, request) {
            ...
            if (this.deliveryManager && routingResult.matched) {
                for (const matchedRoute of routingResult.routes) {
                    if (matchedRoute.matched && !matchedRoute.error) {
                        await this.deliveryManager.deliver(parsedEvent, matchedRoute.route.handler, this.config.delivery);
                    }
                }
            }
            // Step 5: Handle synchronization if sync engine is configured
            if (this.syncEngine && parsedEvent.type !== 'system.event') {
                await this.syncEngine.syncFromAutotask(parsedEvent);
            }
            // Update metrics
    ...
    async checkHealth() {
        ...
        const components = {
            receiver: this.receiver?.isListening()
                ? 'healthy'
                : 'unhealthy',
            parser: 'healthy', // Parser is stateless
            router: 'healthy', // Router is always healthy if initialized
            ...
            eventStore: this.eventStore
                ? 'healthy'
                : 'unhealthy',
            sync: this.syncEngine ? 'healthy' : 'disabled',
        };
        // Check for issues
    ...
    // Metrics methods
    updateReceiverMetrics(success, processingTime) {
