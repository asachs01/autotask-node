...
exports.BusinessRuleEnhancedClient = exports.ValidationError = exports.BusinessRuleMiddleware = void 0;
exports.ValidateEntity = ValidateEntity;
const RuleConfiguration_1 = require("./RuleConfiguration");
...
class BusinessRuleMiddleware {
    constructor(engine, options) {
        ...
        this.options = {
            ...
            errorLogger: options?.errorLogger || new ErrorLogger_1.ErrorLogger()
        };
        this.errorLogger = options?.errorLogger || new ErrorLogger_1.ErrorLogger();
    }
    /**
    ...
    createRequestInterceptor() {
        return async (config) => {
            ...
            // Validate the entity
            if (config.data) {
                const result = await this.validateEntity(entityType, config.data, operation, config);
                if (!result.isValid) {
    ...
    createResponseInterceptor() {
        return async (response) => {
            ...
            if (response.config && response.config.headers) {
                const validationResult = response.config.__validationResult;
                if (validationResult) {
     ...
     * Validate an entity
     */
    async validateEntity(entityType, entity, operation, config) {
        const context = {
            entityType,
            operation,
        ...
        config.__validationResult = result;
        return result;
    }
    /**
     * Handle validation failure
     */
    handleValidationFailure(result, entityType, operation) {
        const context = {
            entityType,
            operation
        };
        ...
        if (this.errorLogger) {
            this.errorLogger.error(`Business rule validation failed for ${entityType} ${operation}`, new Error('Validation failed'), {
                entityType,
                operation,
...
exports.BusinessRuleMiddleware = BusinessRuleMiddleware;
/**
...
function ValidateEntity(options) {
    return function (target, propertyKey, descriptor) {
        ...
        descriptor.value = async function (...args) {
            ...
            const manager = RuleConfiguration_1.RuleConfigurationManager.getInstance();
 ...
 */
class BusinessRuleEnhancedClient {
    constructor(axiosInstance, engine, options) {
        this.axiosInstance = axiosInstance;
        this.engine = engine;
        // Get or create engine
        if (!engine) {
            const manager = RuleConfiguration_1.RuleConfigurationManager.getInstance(options?.errorLogger);
    ...
    setupInterceptors() {
        ...
        this.axiosInstance.interceptors.request.use(this.middleware.createRequestInterceptor(), error => Promise.reject(error));
        // Add response interceptor
        this.axiosInstance.interceptors.response.use(this.middleware.createResponseInterceptor(), error => Promise.reject(error));
    }
    /**
     * Get the business rule engine
     */
...
exports.BusinessRuleEnhancedClient = BusinessRuleEnhancedClient;
//# sourceMappingURL=ClientIntegration.js.map