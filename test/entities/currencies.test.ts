...
describe('Currencies Entity', () => {
  ...
  beforeEach(() => {
    ...
    currencies = new Currencies(mockAxios, mockLogger);
  });
  ...
  describe('list', () => {
    it('should list currencies successfully', async () => {
      ...
      expect(mockAxios.get).toHaveBeenCalledWith('/Currencies/query', {
        params: {
  ...
  describe('get', () => {
    it('should get currencies by id', async () => {
      ...
      const result = await currencies.get(1);

      expect(result.data).toEqual(mockData);
      expect(mockAxios.get).toHaveBeenCalledWith('/Currencies/1');
    });
  });
});